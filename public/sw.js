...
self.addEventListener('notificationclick', (event) => {
  ...
  const { type, postId, groupId, messageId, gameRoomId } = notifData;

  let url = notifData.url || '/';
  if (type === 'new_dm')                 url = `/messages${messageId ? `?ref=${messageId}` : ''}`;
  else if (type === 'new_group_message') url = groupId ? `/groups/${groupId}` : '/groups';
