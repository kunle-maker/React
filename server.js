...
k7nle.post('/api/posts', authenticateToken, upload.array('media', 5), async (req, res) => {
  try {
    const { caption } = req.body;
    const { text, hashtags, isSafe } = processContent(caption);
...
k7nle.get('/api/users/:username', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Optimize profile picture URL if it's from Cloudinary
...
// Keep ONLY this version (the one with proper username lookup)
k7nle.post('/api/messages', authenticateVerifiedToken, async (req, res) => {
  const { receiverUsername, text, formattedText, groupId } = req.body;
  ...
  try {
    if (groupId) {
      ...
      const { text: processedText, isSafe } = processContent(text);
...
k7nle.post('/api/groups/:groupId/messages', authenticateVerifiedToken, async (req, res) => {
  try {
    const { groupId } = req.params;
    const { text, formattedText } = req.body;
    const senderId = req.user.userId;
    ...
    const newMessage = new Message({
      ...
      senderUsername: sender.username,
      text: text.trim(),
      formattedText: formattedText || text.trim(),
      type: 'text',
      status: 'sent',
      read: false
    });
    ...
    // Emit WebSocket event to all group members
    io.to(`group_${groupId}`).emit('groupMessage', {
      message: populatedMessage,
      groupId: groupId
    });
...
k7nle.get('/api/groups/:groupId/messages', authenticateToken, async (req, res) => {
  try {
    ...
    const unreadMessages = messages.filter(msg => 
      ...
      !msg.readBy.some(reader => reader.userId.toString() === userId)
    );
    ...
    const enhancedMessages = messages.map(msg => ({
      ...msg,
      readByMe: msg.readBy.some(reader => reader.userId.toString() === userId),
...
socket.on('sendGroupMessage', async (data) => {
  ...
  socket.on('setOnlineStatus', (isOnline) => {
    if (socketUserId) {
      // Broadcast to all groups user is in
