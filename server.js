...
k7nle.post('/api/login', async (req, res) => {
  try {
    ...
    if (!lastLogin) {
      ...
      const daysDiff = Math.round((today - lastLogin) / (1000 * 60 * 60 * 24));
      if (daysDiff === 0) {
...
k7nle.get('/api/users/me/streak', authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId).select('currentStreak longestStreak lastLoginDate');
    if (!user) return res.status(404).json({ error: 'User not found' });
    res.json({ currentStreak: user.currentStreak || 0, longestStreak: user.longestStreak || 0, lastLoginDate: user.lastLoginDate });
  } catch (err) {
