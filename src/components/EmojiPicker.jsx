...
export default function EmojiPicker({ onSelect, onClose, anchor = 'top' }) {
  ...
  return (
    <div
      ref={pickerRef}
      className="absolute z-50 bg-discord-dark border border-discord-hover rounded-2xl shadow-2xl overflow-hidden select-none"
      style={{
        width: 320,
        ...(anchor === 'top' ? { bottom: 'calc(100% + 8px)' } : { top: 'calc(100% + 8px)' }),
        right: 0,
      }}
      onMouseDown={e => e.preventDefault()}
    >
      {/* Search */}
      <div className="p-2 border-b border-discord-hover">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-discord-hover text-discord-text text-sm pl-7 pr-7 py-1.5 rounded-lg outline-none border border-transparent focus:border-discord-brand/50 placeholder-discord-muted"
          />
          {search && (
      ...
      {/* Category tabs */}
      {!search && (
        <div className="flex items-center gap-0.5 px-2 py-1 border-b border-discord-hover overflow-x-auto scrollbar-none">
