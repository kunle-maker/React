...
export default function ImageCropModal({ src, onCrop, onCancel, circular = false, aspectRatio = 1 }) {
  ...
  const cropH = circular ? CANVAS_SIZE : Math.round(CANVAS_SIZE / aspectRatio);
  const canvasH = cropH;
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const minScale = Math.max(cropW / nw, cropH / nh);
      setScale(minScale + 0.01);
      setOffset({ x: 0, y: 0 });
      setImageLoaded(true);
    };
    img.src = src;
  ...
  }, [dragging, dragStart, naturalSize, scale, canvasH]);
  ...
  const handleConfirm = () => {
    ...
    const outW = 600;
    const outH = circular ? 600 : Math.round(600 / aspectRatio);
    ...
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[60] p-4">
      <div className="bg-discord-dark rounded-2xl overflow-hidden shadow-2xl w-full max-w-sm">
        ...
        <div className="px-4 py-4 border-t border-discord-hover">
          <p className="text-discord-muted text-xs text-center mb-3">Drag to reposition · Pinch or slide to zoom</p>
