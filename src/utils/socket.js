...
class SocketManager {
  constructor() {
    ...
    this.userId = null;
    this._timeout = null;
    // In-memory cache of userId → { isOnline, lastSeen } for the current session
    this.onlineMap = {};
  }
  ...
  _setupListeners() {
    // Always re-join personal room on every (re)connect so the server
    // can route userStatusUpdate events to user_${userId}
    ...
    this.socket.on('userStatusUpdate', (data) => {
      // Keep the in-memory map current so Avatar / status dots can read it synchronously
      if (data?.userId) {
        this.onlineMap[data.userId] = {
    ...
    this.socket.on('pollVoteUpdate', (data) => {
    ...
    this.socket.on('groupMessagesRead', (data) => {
      window.dispatchEvent(new CustomEvent('groupMessagesRead', { detail: data }));
    });
  }
  ...
  /**
   * Tell the server whether this client is foreground/visible or background/hidden.
   * Called from the Page Visibility API handler in App.jsx.
   */
  setOnlineStatus(isOnline) {
    this.socket?.emit('setOnlineStatus', { isOnline });
  }

  /**
   * Synchronously read a cached online status for any userId.
   * Returns null if no update has been received yet in this session.
   */
  getOnlineStatus(userId) {
    return this.onlineMap[userId] ?? null;
  }

  sendMessage(data) { this.socket?.emit('sendMessage', data); }
  sendGroupMessage(data) { this.socket?.emit('sendGroupMessage', data); }
